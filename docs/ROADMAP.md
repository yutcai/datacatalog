...
- **Semantic search**: embed metadata + descriptions with pgvector; answer natural-language queries ("production data from last year in region X"). Embeddings are generated synchronously on write initially (slice A), then moved off the request path by an event consumer once the Kafka backbone lands (slice B).
